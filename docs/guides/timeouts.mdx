...
There are four types of timeouts:
...
The warning timeout is the amount of time (in seconds) before the session
is terminated due to another timeout. It can be used to display a prompt
to the user notifying them of the upcoming timeout, as well as prompting
them to upgrade or extend the session.
