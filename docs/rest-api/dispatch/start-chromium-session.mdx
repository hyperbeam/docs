...
<Param body="timeout" type="object">
  ...
  <Expandable>
      ...
      <Param body="offline" type="number" default="3600">
        The amount of time (in seconds) that no users are connected, after which the session will terminate. Resets when a user connects. Must be greater than 60.
      </Param>
...
<Param body="tag" type="string">
  The `tag` property enforces uniqueness. If a session with tag `"A"` is already running and you attempt to make another
  session with tag `"A"`, the endpoint will not create a new instance and will instead return the `session_id` and `embed_url` of the existing session.
</Param>
